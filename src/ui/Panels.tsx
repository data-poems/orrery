...
export default function Panels(props: PanelProps) {
  ...
  return (
    <>
      ...
      {/* ── Selected comet detail ── */}
      {selComet && (
        <div
          role="dialog"
          aria-label={`${selComet.name} details`}
          style={{
            position: 'absolute', bottom: mobile ? 16 : 56,
            left: '50%', transform: 'translateX(-50%)',
            ...bokehCard, padding: '12px 18px', maxWidth: 420,
        ...
        >
          ...
          {selComet.notable && (
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, lineHeight: 1.6, fontWeight: 300, margin: '6px 0', fontStyle: 'italic' }}>
              {getCometInfo(selComet.name) || 'Notable comet'}
            </p>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 16px', marginTop: 8 }}>
            <Stat label="Perihelion" val={`${selComet.q.toFixed(3)} AU`} c="#66ddff" />
            <Stat label="Eccentricity" val={selComet.e.toFixed(4)} c="#66ddff" />
            <Stat label="Inclination" val={`${selComet.i.toFixed(1)}\u00b0`} />
            <Stat label="Type" val={selComet.e < 1 ? `Periodic (${selComet.type})` : 'Hyperbolic'} />
            {selComet.e < 1 && (
              <Stat label="Period" val={`${((selComet.q / (1 - selComet.e)) ** 1.5).toFixed(1)} yr`} />
            )}
            <Stat label="Abs. magnitude" val={`H ${selComet.H.toFixed(1)}`} />
          </div>
        </div>
      )}

      {/* ── Selected meteor shower detail ── */}
      {selMeteor && (
        <div
          role="dialog"
          aria-label={`${selMeteor.name} details`}
          style={{
            position: 'absolute', bottom: mobile ? 16 : 56,
            left: '50%', transform: 'translateX(-50%)',
            ...bokehCard, padding: '12px 18px', maxWidth: 420,
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ color: '#ffbb33', fontSize: 16 }}>{'\u2726'}</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 16px', marginTop: 8 }}>
            ...
            <Stat label="Velocity" val={`${selMeteor.vg.toFixed(1)} km/s`} c="#ffbb33" />
            <Stat label="Peak (solar lon.)" val={`${selMeteor.solarLonPeak.toFixed(1)}\u00b0`} />
            {selMeteor.parent && <Stat label="Parent body" val={selMeteor.parent} />}
          </div>
        </div>
      )}
      ...
      {selSatellite && (
        <div
          role="dialog"
          aria-label={`${selSatellite.name} details`}
          style={{
            position: 'absolute', bottom: mobile ? 16 : 56,
            left: '50%', transform: 'translateX(-50%)',
            ...bokehCard, padding: '12px 18px', maxWidth: 420,
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 16px', marginTop: 8 }}>
            ...
            <Stat label="Velocity" val={`${selSatellite.vel.toFixed(2)} km/s`} c="#ff66ff" />
            <Stat label="NORAD ID" val={selSatellite.noradId} />
          </div>
        </div>
      )}
